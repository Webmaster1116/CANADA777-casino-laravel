...
function sb_routing_select($exclude_id = false) { 
    $agents = sb_db_get('SELECT id, first_name, last_name FROM sb_users WHERE (user_type = "agent" OR user_type = "admin")' . ($exclude_id ? (' AND id <> ' . $exclude_id) : ''), false);
...
?>
    <div class="sb-main <?php echo ($logged ? 'sb-admin' : 'sb-admin-start') . (sb_get_setting('rtl-admin') ? ' sb-rtl' : '') ?>" style="opacity: 0">
        <?php if ($logged) { ?>
        <div class="sb-header">
                    ...
                    <div class="sb-account">
                        ...
                        <div>
                            ...
                            <ul class="sb-menu">
                                ...
                                <?php if ($is_admin) echo '<li data-value="edit-profile">' . sb_('Edit profile') . '</li>' . (defined('SB_CLOUD') ? ('<li data-value="account">' . sb_('Account') . '</li>') : '') ?>
