...
/*            $checked = new \VanguardLTE\Lib\LicenseDK();
        ...
        public function updateBalance(\Illuminate\Http\Request $request)
        {
            ...
            if ($result == 1){
                return redirect()->back()->withSuccess('Successfully Edit');
            }
            else{
                return redirect()->back()->withErrors('Error occured');
            }
        }
        public function updateBalanceManually(\Illuminate\Http\Request $request)
        {
            $data = $request->all();
            ...
            $result = $user->addBalanceManually($data['type'], $request->summ);
            $result = json_decode($result, true);
        ...
        public function updateDepositAmount(\VanguardLTE\User $user, \Illuminate\Http\Request $request)
        {
            $info = $user->find($user->id);
            $info->max_deposit = $request->daily_deposit_amount;
            $info->save();
            event(new \VanguardLTE\Events\User\UpdatedByAdmin($user));
