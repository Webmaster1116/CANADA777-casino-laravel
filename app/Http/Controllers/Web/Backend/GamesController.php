...
/*            $checked = new \VanguardLTE\Lib\LicenseDK();
            ...
            $apigames = [];
            $games = [];
            $games = \VanguardLTE\Game::select('games.*')->where('shop_id', auth()->user()->shop_id)->orderBy('order', 'ASC');
            $apigames = \VanguardLTE\ApiGames::orderBy('order', 'ASC');
            ...
            if( $request->search != '' ) 
            {
                ...
                $apigames = $apigames->where(function($q) use ($search)
                {
            ...
            if( $request->device != '' ) 
            {
                ...
                if($request->device == '0'){
                    $device = '1';
                }else if($request->device == '1'){
                    $device = '0';
                }else{
                    $device = $request->device;
                }
                $apigames = $apigames->where('mobile', $request->device);
            }
            $apigame_null = "";
            if( $request->gamebank != '' ) 
            {
            ...
            if( $request->label != '' ) 
            {
                ...
                $apigames = $apigames->where('label', $request->label);
            }
            if( $request->jpg != '' ) 
            {
            ...
            if( $savedCategory ) 
            {
                ...
                foreach($savedCategory as $val){

                    $cat = \VanguardLTE\Category::where(['id'=> $val, 'shop_id' => auth()->user()->shop_id])->first();
                    if($cat->href == "livecasino"){
                        $apigames = $apigames->orwhere('type', $cat->href);
                    }else if($cat->href == "jackpot"){
                        $apigames = $apigames->orwhere('name', 'LIKE', '%'.$cat->href.'%');
                    }else{
                        $apigames = $apigames->orwhere('subcategory', '_'.$cat->href);
                    }
                }
            }
            
            $games = $games->get();
            if($apigame_null == "empty") {
                $apigames = [];
            }else {
                
                $apigames = $apigames->get();
                foreach($apigames as $apigame){
                    $apigames_in = \VanguardLTE\UserGamesLog::where(['action'=> 'credit', 'status' => 'NOT_ROLLBACKED', 'game_id'=>$apigame['game_id']])->sum('amount');
                    $apigames_out = \VanguardLTE\UserGamesLog::where(['action'=> 'debit', 'status' => 'NOT_ROLLBACKED', 'game_id'=>$apigame['game_id']])->sum('amount');
                    $apigame['credit_in'] = $apigames_in;
                    $apigame['debit_out'] = $apigames_out;
                }
            }
            ...
            return view('backend.games.list', compact('games', 'apigames', 'views', 'jpgs', 'devices', 'categories', 'emptyGame', 'stats', 'savedCategory'));
        }
        ...
        public function apiedit($apigame)
        {
            $edit = true;
            $id = (int)$apigame;
            $apigame = \VanguardLTE\ApiGames::where('game_id', $id)->first();
            $apigame_in = \VanguardLTE\UserGamesLog::where(['action'=> 'credit', 'status' => 'NOT_ROLLBACKED', 'game_id'=>$id])->sum('amount');
            $apigame_out = \VanguardLTE\UserGamesLog::where(['action'=> 'debit', 'status' => 'NOT_ROLLBACKED', 'game_id'=>$id])->sum('amount');
            return view('backend.games.apiedit', compact('edit', 'apigame', 'apigame_in', 'apigame_out'));
        }
        ...
        public function apiupdate($apigame, \Illuminate\Http\Request $request)
        {
            $id = (int)$apigame;
            if($request->label){
                $api_update = \VanguardLTE\ApiGames::where('game_id', $id)->update(['label'=> $request->label]);
            }else {
                $api_update = \VanguardLTE\ApiGames::where('game_id', $id)->update(['label'=> ""]);
            }
            return redirect()->route('backend.game.apiedit', $apigame)->withSuccess(trans('app.game_updated'));
        }
        ...
        public function orderupdate(\Illuminate\Http\Request $request)
        {
            ...
            if($order_type == "original"){
                if($gametype == "apigame"){
                    \VanguardLTE\ApiGames::where(['game_id' => $gameid])->update(['order' => $order]);
                }else{
