...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    class PagesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function check_freespin100(\Illuminate\Http\Request $request) {
            $status = 0;
            if(\Illuminate\Support\Facades\Auth::check() )
            {
                $userId = \Illuminate\Support\Facades\Auth::id();
                $freespinGame = \VanguardLTE\WelcomePackageLog::where([
                    'user_id' => $userId,
                    'game_id' => 975
                ])->first();
                if($freespinGame && $freespinGame->remain_freespin == 100){
                    $status =  1;
                }
            }
            ...
            if( $detect->isMobile() || $detect->isTablet() ){
                $popupmodal = 'popup-freespin-100-mobile';
            }else{
                $popupmodal = 'popup-freespin-100';
            }
