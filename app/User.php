...
namespace VanguardLTE
{
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        {
            if ($this->wager < 0 || $this->bonus < 0){
                $this->update(['bonus' => 0, 'wager' => 0]);
            }
            if ($this->balance == 0) {
            ...
            $real = $this->balance - $this->bonus;
            if ($real < 0){
                $this->update(['bonus' => $this->balance]);
                $real = 0;
            }
            return $real;
        }
        ...
        {
            // $tmp = $this->bonus - $this->wager / 70;
            $tmp = $this->bonus;
            if ($tmp < 0){
                $this->update(['wager' => $this->bonus * 70]);
                $tmp = 0;
            }
            return $tmp;
        }
        ...
        public function addBalanceManually($type, $summ, $payeer = false, $return = 0)
        {
            if( !in_array($type, [
                'add',
                'out'
