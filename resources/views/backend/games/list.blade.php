                            ...
                            <table class="table table-bordered table-striped">
								...
								@if(count($apigames))
									@foreach ($apigames as $apigame)
										@include('backend.games.partials.apigame_row')
									@endforeach
								@endif
								@if(count($apigames) == 0 && count($apigames) == 0)
...
@section('scripts')
	<script>
		...
		$('.order-update').click(function () {
			...
			var gametype = $(this).prev().attr('data-gametype');
			...
			$.ajax({
				...
				data: {gameid: gameid, order: order, ordertype: ordertype, gametype: gametype},
				success:(data)=>{
					if (data.error) {
						alert(data.msg);
						return;
